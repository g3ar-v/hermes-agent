...
_TTS_PREPROCESS_PROMPT = (
    "You are summarizing an AI agent's response for text-to-speech playback. "
    "Produce a single short sentence summarizing what was done. "
    "Focus on actions taken and outcomes, skip code, URLs, file contents, and "
...
def _build_heuristic_summary(text: str, max_len: int = 400) -> str:
    ...
    # Installs
    installs = re.findall(
        r"(?:installed|added)\s+(\d+)\s+packages?", text, re.I,
    )
    if installs:
    ...
    ran_tests = re.findall(
        r"(?:ran|executed)\s+(\d+)\s+tests?", text, re.I,
    )
    if ran_tests:
